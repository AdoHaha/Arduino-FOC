...
int main() {
  ...
  assert(magneticSensorSPIValidateResponse(valid_angle, true, 14) ==
         MAGNETIC_SENSOR_SPI_READ_OK);

  assert(magneticSensorSPIValidateResponse(valid_angle ^ 0x0020u, true, 14) ==
         MAGNETIC_SENSOR_SPI_PARITY_ERROR);
  assert(magneticSensorSPIValidateResponse(valid_angle ^ 0x8000u, true, 14) ==
         MAGNETIC_SENSOR_SPI_PARITY_ERROR);

  const uint16_t sensor_error = withEvenParity(0x4000u | 0x0234u);
  assert(magneticSensorSPIValidateResponse(sensor_error, true, 14) ==
  ...
  assert(magneticSensorSPIValidateResponse(sensor_error ^ 0x0001u, true, 14) ==
         MAGNETIC_SENSOR_SPI_PARITY_ERROR);

  // Existing generic configurations have the new trailing fields
  // zero-initialized, so neither validation rule is enabled.
  assert(magneticSensorSPIValidateResponse(0xFFFFu, false, 0) ==
         MAGNETIC_SENSOR_SPI_READ_OK);
  return 0;
}
